...
// =========================================================================
__global__ void jacobi_h800_fused_kernel(double* A_global, double* Vt_global, int N_actual, int N_pad, int sweeps) {
    int b = blockIdx.x;        
    int tid = threadIdx.x;     
    int bdim = blockDim.x;     
    ...
    double* A_s = smem;                                  // Size: N * N (约 172.8 KB)
    ...
    for (int s = 0; s < sweeps; ++s) {
        for (int phase = 0; phase < num_phases; ++phase) {
            ...
            // 因为 V_T 是 V 的转置，更新它的行 = 更新原 V 的列。物理地址完全连续！
            for (int idx = tid; idx < total_tasks; idx += bdim) {
                int k = idx / N_actual;
                int j = idx % N_actual;
                int p, q;
...
std::vector<torch::Tensor> jacobi_batched_cuda(torch::Tensor A, int standard_sweeps) {
    ...
    // 
    auto kernel = jacobi_h800_fused_kernel;
    ...
    kernel<<<blocks, threads_per_block, shared_mem_bytes>>>(
        A.data_ptr<double>(), Vt.data_ptr<double>(), N_actual, N_pad, standard_sweeps
    );
...
batched_jacobi = torch.utils.cpp_extension.load_inline(
    name="batched_jacobi_cuda_h800_ultimate",
